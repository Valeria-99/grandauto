...
const Navbar = () => {
  ...
  const changeRole = async ()=> {
    ...
    return (
      ...
      // Navbar container відповідає за підключення навігаційної панелі з адаптивним дизайном
    ...
    ${location.pathname === "/" && "bg-light"}`}>
      ...
      {/* Меню навігації, яке відображає посилання, пошукову панель та кнопки дій */}
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16
        max-sm:border-t border-borderColor ring-0 flex flex-col sm:flex-row 
        ...
        {/* Пошукова панель кнопки Search та Dashboard  */}
            ...
            border-borderColor px-3 rounded-full max-w-56'>
              ...
              <img src={assets.search_icon} alt="searc" />
            </div>

            <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
              <button onClick={()=> isOwner ? navigate('/owner') : changeRole()} className='cursor pointer'>{isOwner 
        ...
        <button className='sm:hiden cursor-pointer' aria-label='Menu' onClick={() => setOpen(!open)}>
